...
@login_required
def training():
    training = db.execute("SELECT * FROM products WHERE category = ?","training")
    return render_template("training.html", training=training)

@app.route("/food", methods=["GET", "POST"])
@login_required
def food():
    food = db.execute("SELECT * FROM products WHERE category = ?", "food")
    return render_template("food.html", food=food)

@app.route("/accessories", methods=["GET", "POST"])
@login_required
def accessories():
    accessories = db.execute("SELECT * FROM products WHERE category = ?", "accessories")
...
def cart():
    ...
    # GET
    cart = db.execute("SELECT * FROM products WHERE productid IN (?)", session["cart"])
